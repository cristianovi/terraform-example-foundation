...
variable "ingress_policies_dry_run_map" {
  description = "Map of custom ingress policies for dry-run perimeter. Map key is the Terraform state key."
  type = map(object({
...
variable "egress_policies_dry_run_map" {
  description = "Map of custom egress policies for dry-run perimeter. Map key is the Terraform state key."
  type = map(object({
    title = optional(string, null)
    from = object({
      sources = optional(object({
...
variable "ingress_policies_map" {
  description = "Map of custom ingress policies for enforced perimeter. Map key is the Terraform state key."
  type = map(object({
    title = optional(string, null)
    from = object({
      sources = optional(object({
...
variable "egress_policies_map" {
  description = "Map of custom egress policies for enforced perimeter. Map key is the Terraform state key."
  type = map(object({
    title = optional(string, null)
    from = object({
      sources = optional(object({
